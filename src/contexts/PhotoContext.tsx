...
import { LIQUOR_IMAGES } from '../data/liquor_images';
...
export function PhotoProvider({ children }: { children: React.ReactNode }) {
  ...
  const getPhotoUrl = useCallback((liquorId: string): string | null | undefined => {
    const userPhoto = photoMap.get(liquorId);
    // User-uploaded photo takes priority; fall back to Open Food Facts catalog image
    if (userPhoto) return userPhoto;
    if (userPhoto === null) {
      // API returned no user photo — check OFF fallback
      const off = LIQUOR_IMAGES[liquorId];
