...
import { setToken, clearToken, apiFetch } from '../api';
...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/social?scope=profiles&action=get&userId=${parsed.id}`).then(res => {
      if (res.status === 404) {
  ...
  const handleCredentialAuth = (newUser: User, token?: string) => {
    if (token) {
      setToken(token);
    }
    handleNewUser(newUser);
