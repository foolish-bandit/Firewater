...
export function useProfile(userId: string | undefined, currentUser: User | null) {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiFetch(`/api/social?scope=profiles&action=get&userId=${userId}`);
  ...
  const updateProfile = useCallback(async (updates: {
    ...
    try {
      const res = await apiFetch(`/api/social?scope=profiles&action=update&userId=${userId}`, {
        method: 'PUT',
  ...
  const toggleFollow = useCallback(async () => {
    if (!currentUser || !userId) return;
    try {
      const res = await apiFetch('/api/social?scope=follows&action=toggle', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
