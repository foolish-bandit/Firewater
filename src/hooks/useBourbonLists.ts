...
export function useBourbonLists(user?: User | null, onError?: (msg: string) => void) {
  ...
  const syncListAction = (bourbonId: string, listType: 'want' | 'tried', action: 'add' | 'remove') => {
    if (!user) return;
    fetch('/api/social?scope=lists&action=update', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-user-id': user.id },
      body: JSON.stringify({ bourbonId, listType, action }),
