                ...
                <button onClick={() => navigate(`/profile/${user.id}`)} className="group">
                  {(() => {
                    ...
                    if (avatarDef) {
                      ...
                      return (
                        <div className="w-8 h-8 rounded-full vintage-border flex items-center justify-center bg-on-surface-accent/15 text-on-surface-accent group-hover:border-border-accent-strong transition-colors">
                          <AvatarComp size={16} />
                        </div>
                      );
                    }
                    if (user.picture) {
                      return <img src={user.picture} alt={user.display_name || user.name} className="w-8 h-8 rounded-full vintage-border group-hover:border-border-accent-strong transition-colors" referrerPolicy="no-referrer" />;
                    }
                    return (
                      <div className="w-8 h-8 rounded-full vintage-border flex items-center justify-center bg-on-surface-accent/20 text-on-surface-accent text-xs font-bold group-hover:border-border-accent-strong transition-colors">
                        {(user.display_name || user.name)?.charAt(0)?.toUpperCase() || <UserIcon size={14} />}
                      </div>
                    );
                  })()}
                </button>
                ...
                >
                  {(() => {
                    ...
                    if (avatarDef) {
                      const AvatarComp = avatarDef.icon;
                      return (
                        <div className="w-8 h-8 rounded-full vintage-border flex items-center justify-center bg-on-surface-accent/15 text-on-surface-accent">
                          <AvatarComp size={16} />
                        </div>
                      );
                    }
                    if (user.picture) {
                      return <img src={user.picture} alt={displayName} className="w-8 h-8 rounded-full vintage-border" referrerPolicy="no-referrer" />;
                    }
                    return (
                  ...
                  <span className="text-sm text-on-surface-muted">{user.display_name || user.name}</span>
                </button>
                <button
