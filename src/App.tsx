...
export default function App() {
  ...
  const { wantToTry, tried, toggleWantToTry, toggleTried } = useBourbonLists(user, showToast);
  const { reviews, addReview, editReview, deleteReview, getReviewsForBourbon } = useReviews(user, showToast);
  ...
  return (
    ...
    <div className="min-h-screen bg-[var(--color-vintage-bg)] text-[var(--color-vintage-text)] font-sans selection:bg-[#C89B3C]/30">
      {/* Navigation */}
      <nav className={`sticky top-0 z-50 bg-[var(--color-vintage-bg)]/95 backdrop-blur-md vintage-border-b transition-shadow duration-300 ${scrolled ? 'shadow-[0_4px_30px_rgba(0,0,0,0.4)]' : ''}`} ref={mobileMenuRef}>
        <div className="max-w-5xl mx-auto px-4 h-20 flex items-center justify-between">
          ...
          >
            <div className="w-10 h-10 rounded-full vintage-border flex items-center justify-center group-hover:border-[#C89B3C] transition-colors overflow-hidden p-1">
              <img src="/logo.svg" alt="BRRL Book Logo" className="w-full h-full object-contain" />
            </div>
            <span className="font-serif text-2xl font-bold tracking-widest text-[#EAE4D9] uppercase">BRRL Book</span>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-3 sm:px-4 py-6 sm:py-8 pb-24 md:pb-8">
        <Suspense fallback={<div className="flex items-center justify-center py-20"><div className="w-8 h-8 border-2 border-[#C89B3C] border-t-transparent rounded-full animate-spin" /></div>}>
        <Routes>
          <Route path="/" element={
            <HomeView
              user={user}
          ...
          <Route path="/feed" element={
            user ? <FeedView user={user} bourbons={allBourbons} /> : <RouterNavigate to="/" replace />
          } />
      ...
      <footer className="max-w-5xl mx-auto px-4 pb-24 md:pb-10 pt-4">
        <div className="vintage-border-t pt-6">
          ...
          <div className="hidden md:block text-center">
            ...
            <p className="text-[#EAE4D9]/40 text-xs font-sans tracking-wide">&copy; 2026 BRRL Book. All rights reserved.</p>
          </div>
          ...
          <div className="md:hidden text-center">
            ...
            <p className="text-[#EAE4D9]/40 text-xs font-sans tracking-wide mt-4">&copy; 2026 BRRL Book. All rights reserved.</p>
          </div>
        </div>
      </footer>
      ...
      {/* Rules Modal */}
      {showRulesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--color-vintage-bg)]/90 backdrop-blur-sm animate-in fade-in duration-300" role="dialog" aria-modal="true" aria-label="Welcome to BRRL Book">
          <div className="bg-[#1A1816] vintage-border p-6 md:p-10 max-w-lg w-full shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="text-center mb-10 relative z-10">
              ...
              <h2 className="font-serif text-4xl font-normal text-[#EAE4D9] mb-4">Welcome to BRRL Book</h2>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#1A1816]/95 backdrop-blur-md border-t border-[var(--color-vintage-border)] safe-bottom">
