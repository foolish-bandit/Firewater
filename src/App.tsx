...
import { List as ListIcon, X, Plus, Menu, Shield, User as UserIcon, Search, Home, ChevronDown, Crosshair, Rss, GitCompareArrows, Sun, Moon, WifiOff } from 'lucide-react';
...
export default function App() {
  ...
  const desktopNavLinkBase = 'relative flex items-center gap-2 py-1 text-xs font-semibold tracking-widest uppercase transition-colors';
  const desktopNavLinkIdle = 'text-theme-muted hover:text-[var(--text-tier-1)]';
  const desktopNavLinkActive = 'text-theme-accent';
  const mobileMenuItemBase = 'block w-full py-4 text-left text-sm font-semibold tracking-widest uppercase transition-colors border-b border-theme';
  const mobileMenuItemIdle = 'text-theme-muted hover:text-[var(--text-accent-strong)]';
  const mobileMenuItemActive = 'text-theme-accent';
  ...
  if (!ageVerified) {
    return (
      <div className="min-h-screen bg-theme-primary flex items-center justify-center p-4">
        <div className="surface-panel elevation-3 p-6 md:p-10 max-w-lg w-full text-center">
          ...
          <p className="micro-label mb-2 text-theme-accent">Age Verification Required</p>
          <h1 className="font-serif text-3xl md:text-4xl font-normal text-theme-primary mb-4">Welcome to FIREWATER</h1>
          <div className="w-12 h-px bg-theme-accent-muted mx-auto mb-6"></div>
          <p className="text-theme-secondary text-sm leading-relaxed mb-8">
            This website contains information about alcoholic beverages. You must be <strong className="text-theme-accent">21 years of age or older</strong> to access this site. By entering, you agree that you are of legal drinking age in your jurisdiction.
          </p>
          ...
          <label className="flex items-center justify-center gap-3 cursor-pointer group mb-8">
            ...
            />
            <span className="text-sm text-theme-secondary group-hover:text-[var(--text-tier-1)] transition-colors">
              I confirm that I am <strong className="text-theme-accent">21 years of age or older</strong>
            </span>
          </label>
          ...
          <button
            ...
            className="btn-base btn-outline w-full py-4 text-sm disabled:hover:bg-transparent disabled:hover:text-[var(--text-accent-strong)] disabled:hover:border-[var(--border-subtle)]"
          >
            Enter Site
          </button>

          <p className="text-theme-faint text-xs mt-6">
  ...
  return (
    <PhotoProvider>
    <div className="min-h-screen bg-theme-primary text-theme-primary font-sans selection:bg-[var(--surface-accent-muted)]">
      {/* Navigation */}
      <nav className={`sticky top-0 z-50 glass-surface vintage-border-b transition-all duration-300 ${scrolled ? 'elevation-1' : ''}`} ref={mobileMenuRef}>
        <div className="max-w-5xl mx-auto px-4 h-20 flex items-center justify-between">
          <div
            className="flex items-center gap-3 cursor-pointer group"
            onClick={() => navigate('/')}
          >
            <div className="w-10 h-10 rounded-full vintage-border flex items-center justify-center group-hover:border-[var(--border-accent-soft)] transition-colors overflow-hidden p-1">
              <img src="/logo.svg" alt="FIREWATER Logo" className="w-full h-full object-contain" />
            </div>
            <span className="font-serif text-2xl font-bold tracking-widest text-theme-primary uppercase">FIREWATER</span>
          </div>
          ...
          <div className="hidden md:flex gap-6 items-center">
            {user && (
              <button
                ...
                className={`${desktopNavLinkBase} ${location.pathname === '/feed' ? desktopNavLinkActive : desktopNavLinkIdle}`}
              >
                <Rss size={14} /> Feed
                {location.pathname === '/feed' && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-theme-accent" />}
              </button>
            )}
            <button
              onClick={() => navigate('/catalog')}
              className={`${desktopNavLinkBase} ${location.pathname === '/catalog' ? desktopNavLinkActive : desktopNavLinkIdle}`}
            >
              Catalog
              {location.pathname === '/catalog' && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-theme-accent" />}
            </button>
            <button
              onClick={() => navigate('/discover')}
              className={`relative text-xs font-semibold tracking-widest uppercase transition-colors flex items-center gap-2 py-1 ${location.pathname === '/discover' ? 'text-[#C89B3C]' : 'text-[#EAE4D9]/60 hover:text-[#EAE4D9]'}`}
            >
              <Crosshair size={14} /> Discover
              {location.pathname === '/discover' && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[#C89B3C]" />}
            </button>
            <button
              onClick={() => navigate('/lists')}
              className={`${desktopNavLinkBase} ${location.pathname === '/lists' ? desktopNavLinkActive : desktopNavLinkIdle}`}
            >
              <ListIcon size={14} /> My Lists
              {location.pathname === '/lists' && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-theme-accent" />}
            </button>
            <button
              onClick={() => navigate('/compare')}
              className={`${desktopNavLinkBase} ${location.pathname === '/compare' ? desktopNavLinkActive : desktopNavLinkIdle}`}
            >
              <GitCompareArrows size={14} /> Compare
              {location.pathname === '/compare' && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-theme-accent" />}
            </button>
            <button
              onClick={() => setShowSubmitModal(true)}
              className={`${desktopNavLinkBase} ${desktopNavLinkIdle}`}
            >
              <Plus size={14} /> Submit
            </button>
            {isAdmin && (
              <button
                ...
                className={`${desktopNavLinkBase} ${location.pathname === '/admin' ? desktopNavLinkActive : desktopNavLinkIdle}`}
              >
                <Shield size={14} /> Admin
                {location.pathname === '/admin' && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-theme-accent" />}
              </button>
            )}
            <button
              onClick={() => setShowUserSearch(true)}
              className="text-theme-muted hover:text-[var(--text-accent-strong)] transition-colors"
              title="Find People"
            >
              <Search size={16} />
            </button>
            ...
            {user ? (
              <div className="flex items-center gap-4">
                <button onClick={() => navigate(`/profile/${user.id}`)} className="group">
                  {user.picture ? (
                    <img src={user.picture} alt={user.name} className="w-8 h-8 rounded-full vintage-border group-hover:border-[var(--border-accent-soft)] transition-colors" referrerPolicy="no-referrer" />
                  ) : (
                    <div className="w-8 h-8 rounded-full vintage-border flex items-center justify-center bg-theme-accent-strong text-theme-accent text-xs font-bold group-hover:border-[var(--border-accent-soft)] transition-colors">
                ...
                <button
                  ...
                  className="text-xs font-semibold tracking-widest uppercase text-theme-muted hover:text-[var(--text-tier-1)] transition-colors"
                >
                  Sign Out
                </button>
              </div>
              ...
              <button
                ...
                className="btn-base btn-outline btn-pill px-5 py-2 text-xs"
              >
                Sign In
              </button>
            )}
        ...
        {mobileMenuOpen && (
          <div
            className="md:hidden fixed inset-0 top-20 bg-theme-overlay backdrop-blur-sm z-40 animate-in fade-in duration-200"
        ...
        {/* Mobile menu */}
        {mobileMenuOpen && (
          <div className="md:hidden bg-theme-surface vintage-border-b px-4 pb-4 elevated-high relative z-50 animate-in slide-in-from-top-2 duration-200">
            <button
              onClick={() => { navigate('/catalog'); setMobileMenuOpen(false); }}
              className={`${mobileMenuItemBase} ${location.pathname === '/catalog' ? mobileMenuItemActive : mobileMenuItemIdle}`}
            >
              Catalog
            </button>
            <button
              onClick={() => { navigate('/discover'); setMobileMenuOpen(false); }}
              className={`block w-full text-left text-sm font-semibold tracking-widest uppercase transition-colors py-4 border-b border-[var(--color-vintage-border)] flex items-center gap-2 ${location.pathname === '/discover' ? 'text-[#C89B3C]' : 'text-[#EAE4D9]/60 hover:text-[#C89B3C]'}`}
            >
              <Crosshair size={14} /> Discover
            </button>
            <button
              onClick={() => { navigate('/lists'); setMobileMenuOpen(false); }}
              className={`${mobileMenuItemBase} ${location.pathname === '/lists' ? mobileMenuItemActive : mobileMenuItemIdle}`}
            >
              My Lists
            </button>
            <button
              onClick={() => { navigate('/compare'); setMobileMenuOpen(false); }}
              className={`${mobileMenuItemBase} flex items-center gap-2 ${location.pathname === '/compare' ? mobileMenuItemActive : mobileMenuItemIdle}`}
            >
              <GitCompareArrows size={14} /> Compare
            </button>
            <button
              onClick={() => { setShowSubmitModal(true); setMobileMenuOpen(false); }}
              className={`${mobileMenuItemBase} ${mobileMenuItemIdle}`}
            >
              Submit
            </button>
            {isAdmin && (
              <button
                ...
                className={`${mobileMenuItemBase} flex items-center gap-2 ${location.pathname === '/admin' ? mobileMenuItemActive : mobileMenuItemIdle}`}
              >
                <Shield size={14} /> Admin
              </button>
            )}
            <button
              ...
              className={`${mobileMenuItemBase} ${mobileMenuItemIdle} flex items-center gap-2`}
            >
            ...
            {user ? (
              <div className="flex items-center justify-between py-4">
                ...
                >
                  {user.picture ? (
                    ...
                    <div className="w-8 h-8 rounded-full vintage-border flex items-center justify-center bg-theme-accent-strong text-theme-accent text-xs font-bold">
                      {user.name?.charAt(0)?.toUpperCase() || '?'}
                    </div>
                  )}
                  <span className="text-sm text-theme-muted">{user.name}</span>
                </button>
                <button
                  onClick={() => { handleSignOut(); setMobileMenuOpen(false); }}
                  className="text-sm font-semibold tracking-widest uppercase text-theme-muted hover:text-[var(--text-accent-strong)] transition-colors"
                >
                  Sign Out
                </button>
              </div>
              ...
              <button
                ...
                className="block w-full text-left text-sm font-semibold tracking-widest uppercase transition-colors py-4 text-theme-accent hover:text-[var(--text-accent-strong)]"
              >
                Sign In
              </button>
            )}
