    ...
    return (
      <div className="text-center py-20">
        <p className="text-[#EAE4D9]/60 font-serif italic text-lg">
          Sign in to see what the community is pouring. Your feed shows reviews, wishlists, and tasting check-ins from the people you follow — a living stream of discovery from fellow explorers.
        </p>
      </div>
    );
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl font-normal text-[#EAE4D9]">Feed</h1>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-[#1A1816] vintage-border border-dashed p-8 sm:p-16 text-center relative overflow-hidden">
          <Logo className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 opacity-[0.03] pointer-events-none" />
          <div className="relative z-10">
            <Users size={32} className="text-[#C89B3C]/30 mx-auto mb-4" />
            <h3 className="font-display text-xl text-[#EAE4D9] mb-2">Your feed is waiting</h3>
            <p className="text-[#EAE4D9]/40 font-serif italic text-lg mb-6 max-w-md mx-auto">
              When you follow other explorers, their reviews, wishlists, and tasting check-ins show up right here. It's the best way to discover bottles through people whose taste you trust.
            </p>
            <button
                  ...
                  >
                    {activity.user_picture ? (
                      <img
                        ...
                        className="w-11 h-11 rounded-full border border-[#EAE4D9]/15 object-cover transition-colors group-hover:border-[#C89B3C]/60"
                        referrerPolicy="no-referrer"
                      />
                    ) : (
                      <div className="w-11 h-11 rounded-full border border-[#EAE4D9]/15 flex items-center justify-center bg-[#C89B3C]/20 text-[#C89B3C] text-sm font-bold transition-colors group-hover:border-[#C89B3C]/60">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="text-[10px] text-[#EAE4D9]/35 tracking-[0.28em] uppercase">{relativeTime(activity.created_at)}</span>
                        </div>

                        <div className="space-y-2">
                          <p className="text-sm sm:text-[15px] text-[#EAE4D9]/88 leading-relaxed">
                            <button
                              onClick={() => navigate(`/profile/${activity.user_id}`)}
                              className="font-semibold text-[#EAE4D9] hover:text-[#C89B3C] transition-colors"
                            >
                              {activity.user_name}
                            </button>
                          ...
                          <div className={`rounded-[22px] px-4 py-3 ${style.panel}`}>
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                              <div className="inline-flex items-center gap-2 text-[#EAE4D9]/75">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${style.iconWrap}`}>
                                  <Icon size={15} />
                                </div>
                                <div>
                                  <p className="text-[10px] tracking-[0.28em] uppercase text-[#EAE4D9]/35">Bottle story</p>
                                  <p className="font-display text-base text-[#EAE4D9] leading-tight">{liquor?.distillery || 'Community bottle'}</p>
                                </div>
                              </div>

                              {liquor && (
                                <div className="text-[11px] tracking-[0.22em] uppercase text-[#EAE4D9]/40">
                            ...
                            {activity.type === 'review' && activity.text && (
                              <p className="mt-3 text-sm sm:text-[15px] text-[#EAE4D9]/72 font-serif italic leading-relaxed line-clamp-3">
                                “{activity.text}”
                              </p>
                            )}
