...
function getPriceTier(price: number): { label: string; className: string } {
  if (price >= 100) return { label: '$$$$', className: 'text-[#C89B3C] text-shadow-[0_0_8px_rgba(200,155,60,0.3)]' };
  if (price >= 60) return { label: '$$$', className: 'text-[#C89B3C]' };
  if (price >= 30) return { label: '$$', className: 'text-[#EAE4D9]/70' };
  return { label: '$', className: 'text-[#EAE4D9]/50' };
}
...
export default function BourbonCard({ bourbon, onClick, isWanted, isTried, onToggleWant, onToggleTried }: BourbonCardProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/photos?action=bourbon&bourbonId=${encodeURIComponent(bourbon.id)}`)
      .then(r => r.ok ? r.json() : [])
      .then(data => { if (!cancelled && Array.isArray(data) && data.length > 0) setPhotoUrl(data[0].blob_url); })
      .catch(() => {});
  ...
  return (
    <div
      onClick={onClick}
      className="group bg-[#1A1816] vintage-border overflow-hidden cursor-pointer hover:border-[#C89B3C] card-elevated card-elevated-hover transition-all duration-300 ease-out flex flex-col h-full relative hover:-translate-y-1.5"
    >
      {/* Top gold accent bar */}
      ...
      {/* Photo / Placeholder */}
      {photoUrl ? (
        <div className="w-full h-40 overflow-hidden bg-[#141210]">
          <img
            src={photoUrl}
            alt={bourbon.name}
        ...
        <div className="w-full h-32 bg-[#141210]/60 flex flex-col items-center justify-center gap-2 border-b border-[var(--color-vintage-border)]">
          <ImageOff size={22} className="text-[#EAE4D9]/10" />
          <span className="text-[#EAE4D9]/20 text-[9px] font-sans font-semibold tracking-widest uppercase flex items-center gap-1">
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex items-start justify-between mb-2">
          ...
          <div className="flex gap-2 shrink-0">
            <button
              ...
              className={`p-2 rounded-full transition-all duration-300 active:scale-90 ${isWanted ? 'bg-[#C89B3C]/20 text-[#C89B3C] border border-[#C89B3C]/50' : 'bg-[#141210]/80 text-[#EAE4D9]/40 hover:text-[#C89B3C] border border-transparent hover:border-[#C89B3C]/30'}`}
              title="Want to Try"
            ...
            <button
              ...
              className={`p-2 rounded-full transition-all duration-300 active:scale-90 ${isTried ? 'bg-[#C89B3C]/20 text-[#C89B3C] border border-[#C89B3C]/50' : 'bg-[#141210]/80 text-[#EAE4D9]/40 hover:text-[#C89B3C] border border-transparent hover:border-[#C89B3C]/30'}`}
              title="Tried"
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className={`font-serif text-lg italic ml-auto font-medium ${priceTier.className}`}>
            {priceTier.label}
            <span className="text-[#EAE4D9]/40 text-sm ml-1">${bourbon.price}</span>
          </span>
        </div>

        {/* Proof indicator bar */}
        <div className="w-full h-[2px] bg-[#141210] rounded-full mb-4 overflow-hidden">
          <div
        ...
        <div className="flex gap-2 mb-4">
          {topFlavors.map(flavor => (
            <span key={flavor} className="px-2 py-0.5 text-[9px] font-sans font-medium tracking-wider uppercase text-[#EAE4D9]/50 border border-[#EAE4D9]/10 rounded-full">
              {flavor}
            </span>
          ))}
        </div>
