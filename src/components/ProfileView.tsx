...
export default function ProfileView({ user, bourbons }: ProfileViewProps) {
  ...
  // Compute Flavor DNA - aggregate flavor profile across tried bourbons
  const flavorDNA = useMemo(() => {
    const triedBourbons = triedIds.map(id => bourbons.find(b => b.id === id)).filter(Boolean) as Bourbon[];
  ...
  // Compute proof badge from tried bourbons
  const proofBadge = useMemo(() => {
    const triedBourbons = triedIds.map(id => bourbons.find(b => b.id === id)).filter(Boolean) as Bourbon[];
    if (triedBourbons.length === 0) return null;
    const avgProof = triedBourbons.reduce((sum, b) => sum + b.proof, 0) / triedBourbons.length;
    return { ...getProofBadge(avgProof), avgProof: Math.round(avgProof) };
  }, [triedIds, bourbons]);

  // Top shelf bourbons
  const topShelfBourbons = useMemo(() => {
    if (!profile?.top_shelf) return [];
    return profile.top_shelf
      .map(id => bourbons.find(b => b.id === id))
      .filter(Boolean) as Bourbon[];
  }, [profile?.top_shelf, bourbons]);

  // Recent reviews with bourbon data
  const recentReviews = useMemo(() => {
    return reviews.slice(0, 20).map(r => ({
      ...r,
      bourbon: bourbons.find(b => b.id === r.bourbonId),
    }));
  }, [reviews, bourbons]);
          ...
          {topShelfBourbons.length > 0 && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                {topShelfBourbons.map((bourbon, i) => (
                  <button
                    key={bourbon.id}
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-[10px] font-sans tracking-wider text-[#C89B3C]/60 mt-1">
                        {bourbon.proof} proof · ${bourbon.price}
                      </p>
                    </div>
                  </button>
                ))}
          ...
          {flavorDNA && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-[#1A1816] vintage-border p-5 sm:p-8">
                <h3 className="micro-label text-[#C89B3C] mb-4 sm:mb-6">Flavor DNA</h3>
                <p className="text-[#EAE4D9]/40 text-xs mb-4 font-serif italic">Aggregate palate based on {triedIds.length} tasted bourbon{triedIds.length !== 1 ? 's' : ''}</p>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        {review.bourbon && (
                          <button
                            onClick={() => navigate(`/bourbon/${review.bourbon!.id}`)}
                        ...
                        {review.bourbon && (
                          <p className="text-[10px] font-sans tracking-widest uppercase text-[#EAE4D9]/40 mt-1">
                            {review.bourbon.distillery} · {review.bourbon.proof} proof
                          </p>
                        )}
                      </div>
