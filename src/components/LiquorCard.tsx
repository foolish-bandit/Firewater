...
function getPriceTier(price: number): { label: string; className: string } {
  if (price >= 100) return { label: '$$$$', className: 'text-theme-accent drop-shadow-[0_0_8px_rgba(200,155,60,0.3)]' };
  if (price >= 60) return { label: '$$$', className: 'text-theme-accent' };
...
export default function LiquorCard({ liquor, onClick, isWanted, isTried, onToggleWant, onToggleTried }: LiquorCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="group surface-card surface-card-hover overflow-hidden cursor-pointer transition-all duration-300 ease-out flex flex-col h-full relative hover:-translate-y-1.5"
    >
      <div className="h-[2px] w-[30%] group-hover:w-full bg-gradient-to-r from-[#C89B3C] to-[#E8C56D] transition-all duration-500 ease-out" />

      {/* Photo / Placeholder */}
      {photoUrl ? (
        <div className="w-full h-40 overflow-hidden bg-theme-primary">
          <img
            src={photoUrl}
            alt={liquor.name}
        ...
        <div className="w-full h-32 bg-theme-overlay flex flex-col items-center justify-center gap-2 border-b border-theme">
          <ImageOff size={22} className="text-theme-quiet opacity-40" />
          <span className="text-theme-quiet text-[9px] font-sans font-semibold tracking-widest uppercase flex items-center gap-1">
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex items-start justify-between gap-4 mb-3 min-h-[4.5rem]">
          <div className="min-w-0 flex-1 space-y-1.5">
            <h3 className="font-serif text-2xl font-normal text-[var(--text-primary)] group-hover:text-[var(--text-accent)] transition-colors leading-tight line-clamp-2">{liquor.name}</h3>
            <p className="micro-label text-[var(--text-accent)] line-clamp-1">{liquor.distillery}</p>
          </div>
          <div className="flex gap-2 shrink-0 pt-0.5">
            <button
              onClick={onToggleWant}
              className={`icon-toggle p-2 active:scale-90 ${isWanted ? 'is-active' : ''}`}
              title="Want to Try"
            ...
            <button
              ...
              className={`icon-toggle p-2 active:scale-90 ${isTried ? 'is-active' : ''}`}
              title="Tried"
