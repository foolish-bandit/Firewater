...
function getTopFlavors(bourbon: Bourbon, count: number = 3): string[] {
...
export default function DetailView({ wantToTry, tried, toggleWantToTry, toggleTried, getReviewsForBourbon, onAddReview, onEditReview, onDeleteReview, user, bourbons }: DetailViewProps) {
  ...
  if (!id || !bourbon) return <div className="text-center py-20 text-[#EAE4D9]/40 font-serif italic text-xl">Bourbon not found</div>;
  ...
  const handleShare = async () => {
    const shareData = {
      title: bourbon.name,
      text: `Check out ${bourbon.name} on FIREWATER`,
  ...
  const handleSubmitReview = (e: React.FormEvent) => {
    ...
    if (reviewMode === 'tasting') {
      onAddReview({ bourbonId: id, rating, text: '', nose, palate, finish, tags: selectedTags.length > 0 ? selectedTags : undefined });
      setNose('');
      setPalate('');
      setFinish('');
    } else {
      onAddReview({ bourbonId: id, rating, text: reviewText, tags: selectedTags.length > 0 ? selectedTags : undefined });
      setReviewText('');
    }
    setRating(0);
    setSelectedTags([]);
  ...
  return (
    <div className="space-y-12 animate-in slide-in-from-right-8 duration-500">
        ...
        <div className="relative z-10 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 sm:gap-6">
            <div className="flex flex-col gap-2">
              ...
              <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl font-normal text-[#EAE4D9] leading-none">{bourbon.name}</h1>
            </div>
            {avgRating && (
          ...
          {/* Photo upload */}
          <div className="mt-4">
            <PhotoUpload bourbonId={id} user={user} />
          </div>
        </div>
      </div>
      ...
      {/* Community Photos */}
      <PhotoGallery bourbonId={id} />

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatBox label="Region" value={bourbon.region} icon={MapPin} />
        <StatBox label="Proof" value={bourbon.proof} icon={Flame} />
        <StatBox label="Age" value={bourbon.age} icon={Clock} />
        <StatBox label="Price" value={`$${bourbon.price}`} icon={DollarSign} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#1A1816] vintage-border p-5 sm:p-8">
          ...
          <p className="text-[#EAE4D9] font-serif text-xl italic">{bourbon.mashBill}</p>
          {bourbon.mashBillDetail && (
            <p className="text-[#EAE4D9]/50 text-sm mt-3 font-serif italic">{bourbon.mashBillDetail}</p>
          )}
        </div>
      ...
      <div className="bg-[#1A1816] vintage-border p-5 sm:p-8">
        <h3 className="micro-label text-[#C89B3C] mb-4 sm:mb-8">Flavor Profile</h3>
