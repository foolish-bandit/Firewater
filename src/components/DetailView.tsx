  ...
  return (
    <div className="space-y-12 animate-in slide-in-from-right-8 duration-500">
      ...
      <div className="relative">
        {/* Proof watermark */}
        <div className="absolute -top-4 right-0 font-serif text-[80px] sm:text-[120px] md:text-[180px] leading-none text-theme-primary opacity-[0.03] select-none pointer-events-none overflow-hidden">
          {liquor.proof}
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 sm:gap-6">
            <div className="flex flex-col gap-2">
              {liquor.source === 'community' && (
                <span className="badge badge-accent self-start rounded-sm">
                  Community Submission
                </span>
              )}
              <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl font-normal text-theme-primary leading-none">{liquor.name}</h1>
            </div>
            {avgRating && (
              <div className="flex items-center gap-2 surface-panel px-4 py-2 self-start shrink-0">
                <Star size={18} className="fill-[var(--text-accent-strong)] text-theme-accent" />
                <span className="font-serif text-xl italic text-theme-primary">{avgRating}</span>
                <span className="text-theme-faint text-xs">({reviews.length})</span>
              </div>
            )}
          </div>
          <p className="micro-label text-theme-accent">{liquor.distillery}</p>

          {/* Action buttons */}
          <div className="grid grid-cols-4 gap-2 sm:gap-4 max-w-lg">
            <button
              onClick={() => toggleWantToTry(id)}
              className={`btn-base py-3 sm:py-4 text-[10px] sm:text-xs ${isWanted ? 'icon-toggle is-active border-[var(--border-accent-strong)] bg-theme-accent-muted text-theme-accent' : 'btn-ghost border border-[var(--border-subtle)]'}`}
            >
            ...
            <button
              ...
              className={`btn-base py-3 sm:py-4 text-[10px] sm:text-xs ${isTried ? 'icon-toggle is-active border-[var(--border-accent-strong)] bg-theme-accent-muted text-theme-accent' : 'btn-ghost border border-[var(--border-subtle)]'}`}
            >
            ...
            <button
              ...
              className="btn-base btn-ghost border border-[var(--border-subtle)] py-3 sm:py-4 text-[10px] sm:text-xs"
            >
      ...
      {/* Description */}
      <p className="text-theme-secondary text-lg leading-relaxed font-serif italic">{liquor.description}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="surface-panel p-5 sm:p-8">
          <h3 className="micro-label text-theme-accent mb-3">Mash Bill</h3>
          <p className="text-theme-primary font-serif text-xl italic">{liquor.mashBill}</p>
          {liquor.mashBillDetail && (
            <p className="text-theme-faint text-sm mt-3 font-serif italic">{liquor.mashBillDetail}</p>
          )}
        </div>

        {/* Dominant Flavors */}
        <div className="surface-panel p-5 sm:p-8">
          <h3 className="micro-label text-theme-accent mb-3">Dominant Notes</h3>
          <div className="flex flex-wrap gap-2 mb-4">
            {topFlavors.map(flavor => (
              <span key={flavor} className="badge badge-accent-soft text-xs">
                {flavor}
              </span>
            ))}
          </div>
          <p className="text-theme-faint text-sm font-serif italic">
      ...
      <div className="surface-panel p-5 sm:p-8">
        <h3 className="micro-label text-theme-accent mb-4 sm:mb-8">Flavor Profile</h3>
        <div className="h-[260px] sm:h-[350px] md:h-[450px] w-full">
      ...
      <div className="space-y-8">
        <div className="text-center mb-10">
          <p className="micro-label text-theme-accent mb-2">Explore</p>
          <h2 className="font-serif text-4xl font-normal text-theme-primary">Similar Taste Profiles</h2>
        </div>
      ...
      <div className="space-y-10">
        <div className="flex items-center justify-between">
          <div>
            <p className="micro-label text-theme-accent mb-2">Thoughts</p>
            <h2 className="font-serif text-4xl font-normal text-theme-primary">Reviews</h2>
          </div>
          {avgRating && (
            <div className="text-right">
              ...
              <p className="micro-label text-theme-faint">{avgRating} avg · {reviews.length} review{reviews.length !== 1 ? 's' : ''}</p>
            </div>
          )}
        </div>

        {/* Review form with gold wash */}
        <div className="surface-panel-accent p-5 sm:p-8">
          <h3 className="font-serif text-2xl text-theme-primary mb-4 sm:mb-6">Leave a Review</h3>

          {/* Mode toggle tabs */}
          <div className="segmented-control mb-4 sm:mb-6 self-start w-fit">
            <button
              type="button"
          ...
          <form onSubmit={handleSubmitReview} className="space-y-4 sm:space-y-6">
            ...
            {/* Tag selector */}
            <div>
              <p className="micro-label text-theme-accent mb-2">How'd you drink it?</p>
                ...
                {REVIEW_TAGS.map(tag => {
                  ...
                  return (
                    <button
                      ...
                      className={`badge px-3 py-1.5 text-[10px] transition-all duration-200 ${
                        isSelected
                          ? 'bg-[var(--text-accent-strong)] text-[var(--surface-base)] border-[var(--text-accent-strong)]'
                          : 'badge-muted hover:border-[var(--border-strong)] hover:text-[var(--text-tier-2)]'
                      }`}
                    >
                      {tag}
                    </button>
            ...
            {reviewMode === 'quick' ? (
              <textarea
                ...
                className="input-surface w-full p-4 sm:p-5 min-h-[100px] sm:min-h-[120px] font-serif italic text-base sm:text-lg resize-none focus:outline-none"
              />
            ) : (
              <div className="space-y-3">
                <div>
                  <label className="micro-label text-theme-accent mb-1.5 block">Nose</label>
                  <textarea
                    value={nose}
                    onChange={(e) => setNose(e.target.value)}
                    placeholder="Aroma — what do you smell?"
                    className="input-surface w-full p-3 sm:p-4 min-h-[60px] sm:min-h-[80px] font-serif italic text-base sm:text-lg resize-none focus:outline-none"
                  />
                </div>
                <div>
                  <label className="micro-label text-theme-accent mb-1.5 block">Palate</label>
                  <textarea
                    value={palate}
                ...
                <div>
                  <label className="micro-label text-theme-accent mb-1.5 block">Finish</label>
                  <textarea
                    value={finish}
            ...
            <button
              ...
              className="btn-base btn-accent px-8 py-3 text-sm disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Post Review
            </button>
          </form>
        ...
        <div className="space-y-6">
          {reviews.length === 0 ? (
            <p className="text-theme-faint italic font-serif text-center py-8">No reviews yet. Be the first to review!</p>
          ) : (
            reviews.map((review: Review) => (
              <div key={review.id} className="surface-panel p-4 sm:p-6 space-y-4">
                {editingId === review.id ? (
