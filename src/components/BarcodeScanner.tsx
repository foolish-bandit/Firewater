...
export default function BarcodeScanner({ onResult, onClose, bourbons }: BarcodeScannerProps) {
  ...
  const processUpc = async (upc: string) => {
    ...
    if (localMatch) {
      const bourbon = bourbons.find(b => b.id === localMatch);
      if (bourbon) {
        onResult({ type: 'match', bourbonId: bourbon.id, upc });
        return;
      }
    }
    ...
    if (externalResult && externalResult.productName) {
      // Check if it's a bourbon product
      if (!isLikelyBourbon(externalResult)) {
        setStatus('error');
        setErrorMessage(
          "That doesn't appear to be a bourbon. Try scanning the bottle's barcode."
        );
      ...
      const match = fuzzyMatchBourbon(externalResult.productName, bourbons);
      if (match) {
        saveUpcMapping(upc, match.id);
        onResult({ type: 'match', bourbonId: match.id, upc });
        return;
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--color-vintage-bg)]/90 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-[#1A1816] vintage-border p-6 md:p-8 max-w-md w-full shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="relative z-10">
          <h2 className="font-serif text-3xl text-[#EAE4D9] mb-2">Scan Barcode</h2>
          <p className="text-[#EAE4D9]/60 mb-6">Point your camera at a bourbon bottle's UPC barcode.</p>
          ...
          {mode === 'camera' && status === 'scanning' && (
            <div className="space-y-4">
              <div
                ...
                className="w-full rounded-lg overflow-hidden bg-black/50 min-h-[280px]"
              />
