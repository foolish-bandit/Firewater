...
export default function ProfileEdit({ profile, bourbons, triedIds, onSave, onClose }: ProfileEditProps) {
  ...
  const availableForShelf = triedBourbons
  ...
  const spiritOptions = ['Bourbon', 'Rye', 'Single Malt', 'Scotch', 'Irish Whiskey', 'Japanese Whisky', 'Mezcal', 'Rum', 'Cognac'];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--color-vintage-bg)]/90 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-[#1A1816] vintage-border w-full max-w-lg max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-6 border-b border-[var(--color-vintage-border)]">
          <h3 className="font-serif text-2xl text-[#EAE4D9]">Edit Profile</h3>
          ...
          <div>
            ...
            {topShelf.length > 0 && (
              <div className="space-y-2 mb-3">
                {topShelf.map((id, i) => {
                  const b = bourbons.find(x => x.id === id);
                  if (!b) return null;
                  return (
                    <div key={id} className="flex items-center gap-3 bg-[#141210] vintage-border p-3">
                      <span className="font-serif text-lg text-[#C89B3C]/50">{i + 1}</span>
              ...
              <div className="relative">
                <div className="flex items-center gap-2 bg-[#141210] vintage-border px-3 focus-within:border-[#C89B3C] transition-colors">
                  ...
                  <input
                    ...
                    placeholder="Search your tried bourbons..."
            ...
            {triedBourbons.length === 0 && (
              <p className="text-[#EAE4D9]/40 text-xs font-serif italic mt-2">Mark some bourbons as tried first to add them to your Top Shelf</p>
            )}
          </div>
