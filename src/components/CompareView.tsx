      ...
      <div className="surface-raised p-5 sm:p-8 space-y-4 mb-hero">
        ...
        <div className="flex flex-wrap gap-2 min-h-[40px]">
          ...
          {selectedIds.length === 0 && (
            <p className="text-on-surface-muted font-serif italic text-sm self-center">No liquors selected</p>
          )}
        </div>
        ...
        {selectedIds.length < 3 && (
          <div className="relative" ref={dropdownRef}>
            <div className="relative">
              <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-muted" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => {
                ...
                className="w-full bg-surface-base vintage-border pl-11 pr-4 py-3 text-on-surface placeholder-on-surface-muted/60 focus:outline-none focus:border-border-accent-strong text-sm font-sans"
              />
              <ChevronDown size={16} className={`absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-muted transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} />
            </div>

            {dropdownOpen && filteredLiquors.length > 0 && (
              <div className="absolute z-20 w-full mt-1 surface-raised max-h-60 overflow-y-auto shadow-xl">
                {filteredLiquors.map(b => (
                  <button
                    key={b.id}
                    onClick={() => addLiquor(b.id)}
                    className="w-full text-left px-4 py-3 hover:bg-on-surface-accent/10 transition-colors border-b border-border-subtle last:border-0"
                  >
                    <span className="text-sm font-sans text-on-surface">{b.name}</span>
                    <span className="text-xs text-on-surface-muted ml-2 font-sans">{b.distillery}</span>
                  </button>
                ))}
              </div>
            )}
      ...
      {selectedLiquors.length >= 2 && (
        <div className="flex flex-col gap-subsection">
                ...
                {flavorDiffs.slice(0, 3).map((diff) => (
                  <div key={diff.key} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-sans font-semibold tracking-[0.25em] uppercase text-on-surface-muted">{diff.label}</span>
                      <span className="text-[10px] font-sans tracking-widest uppercase text-on-surface-accent">{diff.diff.toFixed(1)} pt spread</span>
                    </div>
                    <div className="space-y-2">
                      {selectedLiquors.map((liquor, index) => (
                        <div key={`${diff.key}-${liquor.id}`} className="space-y-1">
                          <div className="flex items-center justify-between text-[10px] font-sans">
                            <span className="text-on-surface-muted">{liquor.name}</span>
              ...
              <table className="w-full text-sm font-sans">
                <thead>
                  <tr className="border-b border-border-subtle">
                    <th className="text-left py-3 pr-4 text-on-surface-muted font-semibold tracking-widest uppercase text-xs">Attribute</th>
                    {selectedLiquors.map((b, i) => (
                ...
                <tbody>
                  {[
                    ...
                    <tr key={row.label} className="border-b border-border-subtle">
                      <td className="py-3 pr-4 text-on-surface-muted font-semibold tracking-widest uppercase text-xs">{row.label}</td>
                      {selectedLiquors.map((b) => (
                        <td key={b.id} className="py-3 px-4 text-on-surface-secondary font-serif italic">{row.getValue(b)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="md:hidden space-y-4">
              {[
                ...
                <div key={row.label} className="border-b border-border-subtle pb-3">
                  <p className="text-on-surface-muted font-semibold tracking-widest uppercase text-[10px] mb-2">{row.label}</p>
                  <div className="space-y-1">
                    {selectedLiquors.map((b, i) => (
                      <div key={b.id} className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: COMPARE_COLORS[i] }} />
                        <span className="text-on-surface-secondary font-serif italic text-sm">{row.getValue(b)}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {flavorDiffs.length > 0 && (
            <div className="border-t border-border-subtle pt-6">
              <h3 className="micro-label text-on-surface-accent mb-4 sm:mb-6">Biggest Flavor Differences</h3>
              <div className="space-y-4">
                {flavorDiffs.slice(0, 6).map((d) => (
                  <div key={d.key} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-sans font-semibold tracking-widest uppercase text-on-surface-muted">{d.label}</span>
                      <span className="text-xs font-sans text-on-surface-accent">
                      ...
                      {selectedLiquors.map((b, i) => (
                        <div key={b.id} className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: COMPARE_COLORS[i] }} />
                            <span className="text-[10px] font-sans text-on-surface-muted truncate">{b.name}</span>
                          </div>
                          ...
                          <span className="text-[10px] font-sans text-on-surface-muted mt-1 block">{d.values[i]}/10</span>
                        </div>
                      ))}
                    </div>
                  </div>
