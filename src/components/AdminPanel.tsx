...
export default function AdminPanel({ user, isAdmin, bourbons }: AdminPanelProps) {
  ...
  const getBourbonName = (bourbonId: string) => {
    const bourbon = bourbons.find(b => b.id === bourbonId);
    return bourbon?.name || 'Unknown Bourbon';
  };
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <div className="text-center space-y-4 py-8">
        <div className="flex items-center justify-center gap-3 mb-2">
          ...
          <h1 className="font-serif text-4xl md:text-5xl font-normal text-[#EAE4D9]">Admin Panel</h1>
        </div>
        <p className="text-[#EAE4D9]/60 font-serif italic text-lg">Review and approve community-submitted bourbon photos.</p>
      </div>
          ...
          {pendingPhotos.map(photo => (
            <div key={photo.id} className="bg-[#1A1816] vintage-border overflow-hidden group">
              ...
              <div className="p-4 space-y-3">
                <div>
                  <button
                    onClick={() => navigate(`/bourbon/${photo.bourbon_id}`)}
                    className="font-serif text-lg text-[#EAE4D9] hover:text-[#C89B3C] transition-colors text-left"
                  >
                    {getBourbonName(photo.bourbon_id)}
                  </button>
