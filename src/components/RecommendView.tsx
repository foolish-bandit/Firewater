...
import { getFlavorVector, cosineSimilarity } from '../utils/bourbonUtils';
...
const QUIZ_QUESTIONS: QuizQuestion[] = [
  {
    id: 'sweetSpice',
    question: 'Do you prefer your bourbon...',
    answers: [
...
function filterByPriceProof(bourbons: Bourbon[], priceFilter: string, proofFilter: string): Bourbon[] {
  return bourbons.filter(b => {
...
function getRecommendations(
  ...
  limit: number
): { bourbon: Bourbon; score: number }[] {
  const targetVec = getFlavorVector(targetProfile);
  const scored = bourbons
...
function computeAverageProfile(bourbons: Bourbon[]): FlavorProfile {
  ...
  for (const key of keys) {
    const sum = bourbons.reduce((acc, b) => acc + b.flavorProfile[key], 0);
    avg[key] = bourbons.length > 0 ? sum / bourbons.length : 5;
  }
...
function ResultsView({
  ...
  recommendations: { bourbon: Bourbon; score: number }[];
    ...
    <div className="animate-in fade-in duration-700 space-y-12">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {recommendations.map(({ bourbon, score }) => (
            <RecommendationCard
              key={bourbon.id}
              bourbon={bourbon}
              score={score}
...
export default function RecommendView({ bourbons, wantToTry, tried }: RecommendViewProps) {
  const hasTriedBourbons = tried.length > 0;
  ...
  const forYouData = useMemo(() => {
    if (!hasTriedBourbons) return null;

    const triedBourbons = bourbons.filter((b) => tried.includes(b.id));
    if (triedBourbons.length === 0) return null;

    const avgProfile = computeAverageProfile(triedBourbons);
    const excludeIds = new Set([...tried, ...wantToTry]);
    const recs = getRecommendations(avgProfile, bourbons, excludeIds, 8);

    return { profile: avgProfile, recommendations: recs };
  }, [bourbons, tried, wantToTry, hasTriedBourbons]);
  ...
  const quizData = useMemo(() => {
    ...
    let candidates = filterByPriceProof(bourbons, priceFilter, proofFilter);
  ...
  }, [quizComplete, quizAnswers, bourbons, tried, wantToTry]);
  ...
  // Mode A: "For You" with tried bourbons (and not switched to quiz)
      ...
      {quizStep === 0 && (
        <div className="text-center space-y-4 mb-12">
          ...
          <p className="text-[#EAE4D9]/60 font-serif italic text-lg max-w-md mx-auto">
            Answer five quick questions and we'll match you with the perfect bourbon.
          </p>
        </div>
      )}
