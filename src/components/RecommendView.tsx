      ...
      {Array.from({ length: total }).map((_, i) => (
        <div
          ...
          className={`rounded-full transition-all duration-300 ${
            i === current
              ? 'w-3 h-3 bg-[var(--text-accent-strong)]'
              : i < current
              ? 'w-2 h-2 bg-[var(--surface-accent-strong)]'
  ...
  return (
    <div className="w-full max-w-xl mx-auto animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="text-center mb-10">
        <p className="micro-label text-theme-accent mb-3">
          Question {currentStep + 1} of {totalSteps}
        </p>
        <h2 className="font-serif text-3xl md:text-4xl text-theme-primary leading-tight">
          {question.question}
        </h2>
      </div>
      ...
      <div className="space-y-4">
        {question.answers.map((answer) => (
          <button
            ...
            className={`w-full p-6 surface-card surface-card-hover text-left transition-all duration-300 group hover:-translate-y-0.5 ${
              selectedValue === answer.value
                ? 'border-[var(--border-accent-strong)] bg-theme-accent-muted'
                : ''
            }`}
          >
            <div className="flex items-center justify-between">
              <span className="font-serif text-xl text-theme-primary group-hover:text-[var(--text-accent-strong)] transition-colors">
                {answer.label}
              </span>
              <ChevronRight
                size={18}
                className="text-theme-quiet group-hover:text-[var(--text-accent-strong)] group-hover:translate-x-1 transition-all"
              />
            </div>
          </button>
        ))}
      ...
      {onBack && (
        <button
          onClick={onBack}
          className="mt-8 flex items-center gap-2 text-theme-faint hover:text-[var(--text-accent-strong)] transition-colors text-sm font-semibold tracking-widest uppercase mx-auto"
        >
...
function RecommendationCard({
  ...
  return (
    <div
      onClick={onClick}
      className="group surface-card surface-card-hover overflow-hidden cursor-pointer transition-all duration-500 flex flex-col h-full relative hover:-translate-y-1"
    >
      {/* Top accent bar */}
      ...
      <div className="absolute top-4 right-4 z-10">
        <div className="badge badge-accent backdrop-blur-sm px-3 py-1.5">
      ...
      <div className="p-6 flex-1 flex flex-col">
        <h3 className="font-serif text-2xl font-normal text-theme-primary group-hover:text-[var(--text-accent-strong)] transition-colors leading-tight mb-2 pr-24">
          {liquor.name}
        </h3>

        <p className="micro-label text-theme-accent mb-3">{liquor.distillery}</p>

        <div className="flex items-center gap-3 mb-3">
          <span className="badge badge-surface text-[10px] rounded-sm">
            {liquor.proof} Proof
          </span>
          <span className="font-serif text-lg italic text-theme-secondary">
            ${liquor.price}
          </span>
        </div>
        ...
        <div className="flex gap-2 mb-4">
          {topFlavors.map((flavor) => (
            <span
              key={flavor}
              className="badge badge-muted text-[9px]"
            >
              {flavor}
            </span>
          ))}
        ...
        <p className="text-sm text-theme-muted line-clamp-2 flex-1 font-serif italic leading-relaxed">
          {liquor.description}
        </p>

        <div className="flex items-center gap-2 text-theme-accent text-xs font-semibold tracking-widest uppercase mt-4 group-hover:gap-3 transition-all">
...
function FlavorRadar({ profile }: { profile: FlavorProfile }) {
  ...
  const highlights = useMemo(() => getSortedFlavorEntries(profile).slice(0, 3), [profile]);
  const summary = useMemo(() => getFlavorSummary(profile), [profile]);

  return (
    <div className="w-full max-w-md mx-auto">
      <div className="text-center mb-4">
        <p className="micro-label text-theme-accent mb-2">Your Flavor Profile</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        {highlights.map((flavor, index) => (
          <div key={flavor.key} className="bg-[#141210] border border-[#EAE4D9]/10 rounded-sm p-3 text-left">
            ...
            <p className="mt-2 text-xs text-[#EAE4D9]/55">{flavor.value.toFixed(1)}/10 intensity</p>
          </div>
        ))}
      </div>
      <div className="md:hidden space-y-3 mb-5">
        {highlights.map((flavor) => (
      ...
      <p className="micro-label text-[#EAE4D9]/35 mb-3 text-center">Radar chart for deeper detail</p>
      ...
      <div className="md:hidden h-[220px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart cx="50%" cy="50%" outerRadius="65%" data={data}>
            ...
            <PolarAngleAxis
              dataKey="subject"
              tick={{ fill: 'rgba(234, 228, 217, 0.32)', fontSize: 9, fontFamily: 'Montserrat' }}
            />
            <Radar
              name="Your Palate"
              dataKey="A"
  ...
  return (
    <div className="animate-in fade-in duration-700 space-y-12">
      ...
      <div className="text-center space-y-3">
        <div className="w-14 h-14 rounded-full vintage-border flex items-center justify-center mx-auto mb-4 text-theme-accent">
          <Sparkles size={24} />
        </div>
        <h1 className="font-serif text-4xl md:text-5xl text-theme-primary">{title}</h1>
        <p className="text-theme-muted font-serif italic text-lg">{subtitle}</p>
      </div>

      {/* Radar Chart */}
      <div className="surface-panel p-6 max-w-lg mx-auto">
      ...
      <div>
        <div className="text-center mb-8">
          <p className="micro-label text-theme-accent mb-2">Top Picks</p>
          <h2 className="font-serif text-3xl text-theme-primary">We Think You'll Love</h2>
        </div>
        ...
        {recommendations.length === 0 && (
          <div className="text-center py-16">
            <p className="text-theme-faint font-serif italic text-lg">
      ...
      {onReset && (
        <div className="text-center pt-4">
          <button
            onClick={onReset}
            className="btn-base btn-outline px-8 py-4 text-sm"
          >
    ...
    return (
      <div className="min-h-[80vh] animate-in fade-in duration-700 space-y-12">
        <ResultsView
          ...
          subtitle="Based on your tasting history and discovery path"
        />

        {/* Also offer the quiz */}
        <div className="section-divider" />
        <div className="text-center space-y-4 pb-8">
          <p className="text-theme-faint font-serif italic">
          ...
          <button
            ...
            className="btn-base btn-outline px-8 py-3 text-sm"
          >
            <ArrowRight size={16} /> Start Discovery Quiz
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-[80vh]">
        <ResultsView
          ...
          title="Your Discovery Matches"
          subtitle="Based on your flavor preferences and discovery answers"
        />
      </div>
    );
  }
    ...
    <div className="min-h-[80vh] flex flex-col items-center pt-8 md:pt-16 animate-in fade-in duration-700">
      ...
      {quizStep === 0 && (
        <div className="text-center space-y-4 mb-12">
          ...
          <h1 className="font-serif text-4xl md:text-5xl text-theme-primary">Flavor Quiz</h1>
          <p className="text-theme-muted font-serif italic text-lg max-w-md mx-auto">
