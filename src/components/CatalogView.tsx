...
export default function CatalogView({ wantToTry, tried, toggleWantToTry, toggleTried, bourbons, onOpenSubmit, onOpenScanner }: CatalogViewProps) {
  ...
  const flavorKeywords: Record<string, keyof Bourbon['flavorProfile']> = {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    // Score each bourbon
    const scored = bourbons.map((b: Bourbon) => {
      let score = 0;
  ...
  const filteredBourbons = useMemo(() => {
    ...
    if (searchResults) {
      result = searchResults.map((id: string) => bourbons.find((b: Bourbon) => b.id === id)).filter(Boolean) as Bourbon[];
    }
  ...
  }, [searchResults, maxPrice, minProof, bourbons, activeCategory]);

  const sortedBourbons = useMemo(() => {
    const sorted = [...filteredBourbons];
    switch (sortBy) {
    ...
    return sorted;
  }, [filteredBourbons, sortBy]);

  useEffect(() => setPage(1), [searchQuery, searchResults, activeCategory, maxPrice, minProof, sortBy]);

  const totalPages = Math.ceil(sortedBourbons.length / ITEMS_PER_PAGE);
  const paginatedBourbons = sortedBourbons.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE);
      ...
      <div className="bg-[#1A1816] vintage-border p-4 sm:p-6 space-y-4 sm:space-y-6">
        <div className="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
           <div className="flex items-baseline gap-3">
             ...
             <span className="micro-label text-[#C89B3C]">{sortedBourbons.length} bourbons</span>
           </div>
          ...
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8">
              <span className="micro-label text-[#C89B3C]">
                Showing {(page - 1) * ITEMS_PER_PAGE + 1}–{Math.min(page * ITEMS_PER_PAGE, sortedBourbons.length)} of {sortedBourbons.length} bourbons
              </span>
