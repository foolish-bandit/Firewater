...
function getTopFlavors(bourbon: Bourbon, count: number = 4): { name: string; value: number }[] {
...
export default function ListCard({ bourbon, onClick }: ListCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="flex items-center gap-4 bg-[#1A1816] vintage-border p-4 cursor-pointer hover:border-[#C89B3C] transition-all duration-300 group"
    >
      ...
      <div className="flex-1 min-w-0">
        <h4 className="font-serif text-lg text-[#EAE4D9] group-hover:text-[#C89B3C] transition-colors truncate">{bourbon.name}</h4>
        <p className="micro-label text-[#C89B3C]">{bourbon.distillery}</p>
        {bourbon.type && <p className="micro-label text-[#EAE4D9]/40 mt-0.5">{bourbon.type}</p>}
      </div>

      <div className="text-right shrink-0">
        <p className="text-sm font-serif text-[#EAE4D9]/60">{bourbon.proof}°</p>
        <p className="text-sm font-serif text-[#EAE4D9]/40 italic">${bourbon.price}</p>
      </div>
