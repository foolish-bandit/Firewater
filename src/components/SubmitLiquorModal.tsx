...
export default function SubmitLiquorModal({ onClose, onSubmit, onSelectExisting, existingLiquors, prefillName, prefillDetails, onOpenScanner }: SubmitLiquorModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--color-vintage-bg)]/90 backdrop-blur-sm animate-in fade-in duration-300" role="dialog" aria-modal="true" aria-label="Submit a Liquor">
      <div ref={modalRef} className="bg-[#1A1816] vintage-border p-6 md:p-8 max-w-2xl w-full shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
        <button
          onClick={onClose}
        ...
        <div>
          <h2 className="font-display text-3xl text-[#EAE4D9] mb-2">Submit a Liquor</h2>
          <p className="text-[#EAE4D9]/60 mb-4">Help grow the community database.</p>
          ...
          <div className="flex items-center gap-2 mb-6">
            {(['input', 'duplicate-check', 'loading', 'review'] as const).map((s, i) => (
              <div key={s} className={`w-2 h-2 rounded-full transition-colors ${
                s === step ? 'bg-[#C89B3C]' :
                (['input', 'duplicate-check', 'loading', 'review'].indexOf(step) > i) ? 'bg-[#C89B3C]/50' :
                'bg-[#EAE4D9]/15'
              }`} />
            ))}
          </div>
          ...
          {step === 'input' && (
            <form onSubmit={handleInputSubmit} className="space-y-6">
              <div>
                <label className="block text-xs font-semibold tracking-widest uppercase text-[#EAE4D9]/60 mb-2">Liquor Name *</label>
                <input
                  type="text"
                  value={inputName}
          ...
          {step === 'duplicate-check' && (
            <div className="space-y-6">
              <div className="bg-[#C89B3C]/10 border border-[#C89B3C]/30 p-4 rounded-lg">
                <h3 className="text-[#C89B3C] font-display text-xl mb-2">Wait, did you mean one of these?</h3>
                <p className="text-[#EAE4D9]/80 text-sm">We found some similar liquors already in the database.</p>
              </div>
          ...
          {step === 'loading' && (
            <div className="py-20 flex flex-col items-center justify-center text-center space-y-6">
              ...
              <div>
                <h3 className="text-xl font-display text-[#EAE4D9] mb-2">Analyzing Liquor Data</h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 max-h-56 overflow-y-auto pr-2 custom-scrollbar">
                      {Object.entries(generatedLiquor.flavorProfile || {}).map(([key, value]) => (
                        <div key={key} className="flex items-center gap-2">
                          <span className="text-xs text-[#EAE4D9]/80 capitalize w-16 shrink-0">{key}</span>
                          <input
                            type="range"
                            min="0"
                            max="10"
                          ...
                          />
                          <span className="text-xs text-[#C89B3C] w-4 text-right">{value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
