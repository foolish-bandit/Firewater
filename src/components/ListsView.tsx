...
function exportLists(wantBourbons: Bourbon[], triedBourbons: Bourbon[]) {
  const lines = ['=== BRRL BOOK — MY LISTS ===', ''];
  ...
  lines.push('');
  lines.push(`Exported from BRRL Book on ${new Date().toLocaleDateString()}`);

  const blob = new Blob([lines.join('\n')], { type: 'text/plain' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'brrl-book-lists.txt';
  a.click();
  URL.revokeObjectURL(url);
}

export default function ListsView({ wantToTry, tried, toggleWantToTry, toggleTried, bourbons, reviews }: ListsViewProps) {
  ...
  const total = wantBourbons.length + triedBourbons.length;
  const completionPct = total > 0 ? Math.round((triedBourbons.length / total) * 100) : 0;

  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      {/* Header */}
      <div className="text-center space-y-4 py-8">
        <h1 className="font-serif text-4xl md:text-5xl font-normal text-[#EAE4D9]">My BRRL Book</h1>
      ...
      {total > 0 && (
        <div className="grid grid-cols-3 gap-2 sm:gap-4">
          <div className="bg-[#1A1816] vintage-border p-3 sm:p-5 flex flex-col items-center text-center">
            ...
            <span className="font-serif text-xl sm:text-2xl text-[#EAE4D9]">{wantBourbons.length}</span>
          ...
          <div className="bg-[#1A1816] vintage-border p-3 sm:p-5 flex flex-col items-center text-center">
            ...
            <span className="font-serif text-xl sm:text-2xl text-[#EAE4D9]">{triedBourbons.length}</span>
          ...
          {triedBourbons.length === 0 ? (
            <div className="bg-[#1A1816] vintage-border border-dashed p-8 sm:p-16 text-center relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-[#EAE4D9]/40 font-serif italic text-lg mb-6">You haven't marked any bourbons as tried yet.</p>
                <button
                  onClick={() => navigate('/catalog')}
                  className="bg-transparent vintage-border hover:bg-[#C89B3C] hover:text-[#141210] hover:border-[#C89B3C] text-[#C89B3C] font-sans font-semibold tracking-widest uppercase px-6 py-3 text-xs transition-all duration-300"
                >
