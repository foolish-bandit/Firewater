...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Bulk imports from system can be auto-approved
    const isBulkImport = req.headers["x-bulk-import"] === "true" && userId === "system-bulk-import";
    const status = isBulkImport ? "approved" : "pending";

    // Store metadata in Postgres
    const id = crypto.randomUUID();
    await sql`
      INSERT INTO photos (id, bourbon_id, user_id, user_email, user_name, blob_url, status, created_at)
      VALUES (${id}, ${bourbonId}, ${userId}, ${userEmail}, ${userName || null}, ${blob.url}, ${status}, NOW())
    `;
