...
function signDevToken(payload: { userId: string; email?: string }): string {
...
async function startServer() {
  ...
  app.get(["/auth/callback", "/auth/callback/"], async (req, res) => {
    ...
    try {
      ...
      const dbUser = rows[0];
      const jwtToken = signDevToken({ userId: dbUser.id, email: dbUser.email });
